<?php
	...
	class Table_RoomList extends Table_ListTemplate{
		...
		protected function addHeader(){
			$row = array("Campus:", "Building:", "Level:", "Room:");
			$this->addRow($row);
		}
		...
		protected function getQuerySelect(){
			return "SELECT room.ID,room.RoomName,room.Level,building.BuildingName,campus.CampusName";
		}
		
		// Get the query condition
		protected function getQueryFrom(){
			$condition = " FROM room JOIN building ON room.BuildingID=building.ID";
			$condition .= " JOIN campus ON building.CampusID=campus.ID";
		...
		protected function getQueryOrder(){
			...
			return " ORDER BY campus.CampusName ASC,building.BuildingName,room.Level,room.RoomName LIMIT ".$tableStart.",".$this->tableSize;
		}
		...
		protected function parseRow($_row){
			...
			$row = array($_row['CampusName'], $_row['BuildingName'], $_row['Level'], $link);
			$this->addRow($row);
		}
	}
?>
